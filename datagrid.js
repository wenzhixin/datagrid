...
define(function(require, exports, module) {
	...
	Datagrid.View = function (options) {
		...
		this.currPage = 1;
		if (!this.collection) {
			throw ('Datagrid: collection is undefined');
		}
	...
	_.extend(Datagrid.View.prototype, Backbone.View.prototype, {
		lang: 'en_US',
		tagName: 'div',
		className: 'datagrid-container',
		setDict: function (lang) {
			if(Datagrid.Dict.hasOwnProperty(lang)) {
				this.lang = lang;
			}
			this.dict = Datagrid.Dict[this.lang];
		},
		...
		render: function () {
			...
			if (!this.$grid) {
				this.$grid = $('<table class="datagrid-grid table table-bordered table-condensed" />');
				if (this.caption) {
			...
			if (!this.$thead) {
				this.thead = new Datagrid.TheadView({view: this});
				this.$thead = this.thead.render();
				this.$grid.append(this.$thead);
			}
			if (!this.$navBar) {
				this.navBar = new Datagrid.NavView({view: this});
				this.$navBar = this.navBar.render();
				this.$grid.after(this.$navBar);
				this.$loading = $('<div class="datagrid-loading progress progress-info progress-striped active"><div class="bar datagrid-loading-progress">' + this.dict.loading + '</div></div>');
				this.$navBar.prepend(this.$loading);
			}
			if (!this.$searchBar && this.enableSearch) {
				this.searchBar = new Datagrid.SearchView({view: this});
		...
		showCollection: function (collection) {
			...
			if (collection.length === 0 && !this.autofetch) {
				this.$grid.append('<tbody><tr class="datagrid-noRows"><td colspan="' + this.colLength + '">' + this.dict.noData + '</td></tr></tbody>');
			}
		},
		...
		toggleSubgridRow: function (model, $el, options) {
			...
			if (View.$subgridContainer) {
				$('td.datagrid-subgrid-control i', View.$subgridContainer.prev()).removeClass('icon-minus');
			...
			$('td.datagrid-subgrid-control i', $el).addClass('icon-minus');
			colspan = this.multiselect ? 2 : 1;
			subgridRow = _.template('<tr class="datagrid-subgrid-row"><td colspan="<%=extra%>"/><td colspan="<%=colspan %>"></td></tr>');
		...
		renderRow: function (model) {
			...
			this.rowViews[model.id] = new Datagrid.RowView({model: model, view: this});
		...
		onSort: function (event) {
			...
			if (!col || (col && (col.name === 'datagrid-actions-cell' || !col.index))) {
				return false;
			}
		...
		onPageChanged: function (event) {
			...
			if (this.currPage !== page) {
				this.currPage = page;
				$('div.datagrid-pager li', this.$el).removeClass('active');
				$('.datagrid-page-input', this.$pager).val(this.currPage);
	
				if (this.currPage === 1) {
					$('div.datagrid-pager a.left,.first', this.$el).parent().addClass('active');
				}
				if (this.currPage >= this.cntPages) {
					$('div.datagrid-pager a.right,.last', this.$el).parent().addClass('active');
				}
	...
	Datagrid.View.extend = Backbone.View.extend;
	
	Datagrid.RowView = function (options) {
		this.events = {
			"click td[class!=datagrid-actions-cell]": "setSelection",
			"dblclick td[class!=datagrid-actions-cell]": "onDblClick"
		};
		Backbone.View.apply(this, [options]);
	...
	_.extend(Datagrid.RowView.prototype, Backbone.View.prototype, {
		tagName: 'tr',
		...
		setSelection: function (options) {
			...
			var target = options.currentTarget || undefined,
				...
				self = this,
				$control = $(target).closest('tr').find('td.datagrid-multiselect-control input');
			if ($control && $control.is(':disabled') && className !== 'datagrid-subgrid-control') {
				return false;
			}
			if (!(this.view.multiselect && this.view.subgrid && className !== 'datagrid-subgrid-control')) {
				this.view.trigger("selected", this.model, this.$el, options);
			}
			if (this.view.multiselect && className === 'datagrid-subgrid-control') {
				return false;
			}
			...
			if (this.view.onRowClick) {
				this.view.onRowClick(this.model, this.selected, options);
			}
		...
		render: function () {
			...
			row = _.template('<% if (isMultiselect) {%>\
				<td class="datagrid-multiselect-control"><input type="checkbox" <% if (isDisabled) { %>disabled="disabled"<% } %><% if (isChecked) {%>checked="checked"<%}%>></td>\
				<%} if (isContainSubgrid) {%>\
					<td class="datagrid-subgrid-control">\
						<i class="icon-plus<%if (isSelected) {%> icon-minus<%}%>">\
					</td>\
					<%} _.each(values, function (row) {%>\
						<td <% if (row.name === "datagrid-actions-cell") {%>class="datagrid-actions-cell"<%}%>>\
							<%=row.value%>\
						</td>\
					<%})%>');
			...
			html = row({
				...
				values: _.map(cols, function (col) {
					if (col.actions) {
						col.name = 'datagrid-actions-cell';
	...
	Datagrid.RowView.extend = Backbone.View.extend;
	
	Datagrid.PagerView = function (options) {
		this.events = {
			'click a': 'onPageChanged',
			'change .datagrid-pager-rowlist': 'onRowsChanged',
			'change .datagrid-page-input': 'onPageChanged'
		};
	...
	_.extend(Datagrid.PagerView.prototype, Backbone.View.prototype, {
		tagName: 'div',
		className: 'datagrid-pager-container span offset',
		...
		initPager: function () {
			...
			var pager = _.template('<div class="span datagrid-pager">\
				<ul class="nav nav-pills">\
					...
					<li>\
						<div class="datagrid-page-counter pull-left">' + this.view.dict.page + '.</div>\
						<input class="datagrid-page-input" value="<%=page%>" type="text">\
						<div class="datagrid-page-counter-right pull-right"> ' + this.view.dict.prep + ' <%=cntpages%> </div>\
					</li>\
				...
				<% if (rowlist) {%>\
					<div class="datagrid-pager-rowlist-label pull-left">' + this.view.dict.rowsOnPage + ':</div>\
					<select class="datagrid-pager-rowlist">\
			...
			if (!this.view.rowList) {
				this.$el.addClass('datagrid-pager-container-norowslist');
			}
	...
	Datagrid.PagerView.extend = Backbone.View.extend;
	
	/* View of thead */
	Datagrid.TheadView = function (options) {
		this.events = {
			'click th': 'onSort',
	...
	_.extend(Datagrid.TheadView.prototype, Backbone.View.prototype, {
		tagName: 'thead',
		className: 'datagrid-grid-head',
		...
		render: function () {
			...
			if (!this.$headHolder) {
				this.$headHolder = $('<tr class="datagrid-grid-head-holder"/>');
			...
			if (this.view.hideHeaderCheckbox) {
				this.$headHolder.find(':checkbox').hide();
			}
			if (!this.view.$filterBar && this.view.enableFilter) {
				this.view.filterBar = new Datagrid.FilterView({ view: this.view });
		...
		resetAllcheckbox: function () {
			var rows = $('.datagrid-row input[type=checkbox]', this.view.$el).length,
				checkedRows = $('.datagrid-row input[type=checkbox]:checked', this.view.$el).length;
			$('input[type=checkbox]', this.$el).prop('checked', checkedRows === rows);
		}
	});
	
	Datagrid.TheadView.extend = Backbone.View.extend;
	
	Datagrid.NavView = function (options) {
		Backbone.View.apply(this, [options]);
		this.view = options.view;
	};
	
	_.extend(Datagrid.NavView.prototype, Backbone.View.prototype, {
		tagName: 'div',
		className: 'datagrid-grid-nav row',
		render: function () {
			if (this.view.buttons) {
				var self = this, btn, btnHtml, $button;
				this.view.$buttonsContainer = $('<div class="datagrid-navBar-buttonsContainer btn-group span"/>');
			...
			if (!this.view.$pager && this.view.rows) {
				this.view.pager = new Datagrid.PagerView({ view: this.view });
	...
	Datagrid.NavView.extend = Backbone.View.extend;
	
	Datagrid.SearchView = function (options) {
		this.events = {
	...
	_.extend(Datagrid.SearchView.prototype, Backbone.View.prototype, {
		tagName: 'div',
		className: 'datagrid-search-bar pull-right',
	...
	Datagrid.SearchView.extend = Backbone.View.extend;
	
	Datagrid.FilterView = function (options) {
		this.events = {
	...
	_.extend(Datagrid.FilterView.prototype, Backbone.View.prototype, {
		tagName: 'tr',
		className: 'datagrid-filter-bar',
		initialize: function (options) {
			options.view._collection = options.view.collection;
		},
	...
	Datagrid.FilterView.extend = Backbone.View.extend;
